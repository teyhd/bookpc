...
namespace Check
{
    class ProcessServices 
    {
        ...
        static public bool StartProcessAsCurrentUser(
        ...
        static private IntPtr GetEnvironmentBlock(IntPtr token)
        {
            IntPtr block = IntPtr.Zero;
            ...
            if (!result)
            {
                Program.Mylog($"CreateEnvironmentBlock Error: {Marshal.GetLastWin32Error()}");
            }
        ...
        static private IntPtr GetPrimaryToken(Process process)
        {
            ...
            if (openResult)
            {
                ...
                if (!openResult)
                {
                    Program.Mylog($"DuplicateTokenEx Error: {Marshal.GetLastWin32Error()}" );
                }
            }
            else
            {
                Program.Mylog($"OpenProcessToken Error: {Marshal.GetLastWin32Error()}" );
            }
        ...
        static private bool LaunchProcess(
            string commandLine,
            ...
            IntPtr environmentBlock)
        {
            ...
            if (!result)
            {
                Program.Mylog($"CreateProcessAsUser Error: {Marshal.GetLastWin32Error()}");
            }
